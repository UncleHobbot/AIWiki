...
def parse_frontmatter(text):
    ...
    # title, title_ru, category, updated, date
    for key in ("title", "title_ru", "category", "updated", "date"):
...
def build_index(records: list) -> dict:
    """Aggregate records into fast-lookup dicts by tag and category."""
    by_tag      = {}
    by_category = {}
...
def main():
    ...
    if args.slug:
        ...
        # Patch the index
        idx = read_json(INDEX_FILE) if INDEX_FILE.exists() else {"by_tag": {}, "by_category": {}}
        # Remove old slug entries, re-add

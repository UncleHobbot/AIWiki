---
name: "source-command-wiki-fix"
description: "Migrated source command `wiki-fix`"
---

# source-command-wiki-fix

Use this skill when the user asks to run the migrated source command `wiki-fix`.
...
3. Fill each missing required field with a sensible value:
   ...
   - `category` — if missing or invalid, infer from content using the
     category decision tree in AGENTS.md; valid values are:
...
3. Russian section rules (from AGENTS.md):
...
`.Codex/commands/wiki-index.md`) to fully rebuild `index.md`.
